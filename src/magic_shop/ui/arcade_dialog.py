...
class ArcadeCanvas(QWidget):
    ...
    def start(self) -> None:
        """Start the animation timer used by the moving target."""

        self.running = True
        self._tick.start(16)

    def stop(self) -> None:
        """Stop animation without resetting the current score."""

        self.running = False
        self._tick.stop()

    def reset(self) -> None:
        """Reset gameplay state so the same dialog can be reused."""
    ...
    def _step(self) -> None:
        """Advance target position and bounce it inside the canvas bounds."""
    ...
    def mousePressEvent(self, event) -> None:  # noqa: N802
        ...
        if inside:
            ...
            self.target_color = QColor(
                randint(60, 255), randint(60, 255), randint(60, 255)
            )
        else:
            self.misses += 1
            self.combo = 1
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        """Paint the game board with a high-contrast grid and target."""

        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        painter.fillRect(self.rect(), QColor("#0f1714"))

        pen = QPen(QColor("#22352f"))
        pen.setWidth(1)
        painter.setPen(pen)
        ...
        painter.drawEllipse(
            self.target_center, self.target_radius * 0.35, self.target_radius * 0.35
        )
...
class ArcadeDialog(QDialog):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.resize(700, 460)
        self.setStyleSheet(ARCADE_STYLE)
        ...
        self.lbl_hint = QLabel(
            "Hit the moving target. Higher combo = more reward stock units."
        )
    ...
    def start_game(self) -> None:
        """Start one timed challenge; the score remains visible after the timer ends."""
    ...
    def restart_game(self) -> None:
        """Reset the timer and game canvas for another attempt."""
    ...
    def _tick_second(self) -> None:
        """Advance the countdown and stop the game on time or miss limit."""
    ...
    def refresh_status(self) -> None:
        """Keep the HUD synchronized with the current score and reward."""
    ...
    def reward_units(self) -> int:
        """Expose the score conversion so MainWindow can apply the stock reward."""
